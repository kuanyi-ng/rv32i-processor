...
\begin{document}
  ...
  \begin{figure}[h]
    ...
    \resizebox{\columnwidth}{!}{
      \includegraphics{../../images/external_interface.png}
    }
  ...
  \begin{itemize}
    ...
    \newline 命令メモリへの $32\unit{\bit}$ のアクセスアドレスパス.

    \item IDT (Instruction DaTa Bus)
    \newline 命令メモリからの $32\unit{\bit}$ のデータパス.
    ...
    \newline データメモリへの $32\unit{\bit}$ のアクセスアドレスパス.

    \item DDT (Data DaTa bu)
    \newline データメモリからの $32\unit{\bit}$ のデータパス.
  ...
  メモリのアドレスは $1\unit{\byte}$ ごとに振り分けられ, 
  1つのアドレスに $32\unit{\bit}$ のデータを保持することができる.
