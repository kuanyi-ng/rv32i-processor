...
\begin{document}
  ...
  % TODO: 機能検証の項目では方法と結果が1つの節で書かれているのに、この後の性能評価では方法と結果が別の節で書かれていて、書き方がぶれている
  \subsection{性能評価方法}
  設計したプロセッサの性能を, プログラム実行のクロックサイクル数, 
  最小動作クロック周期, 面積, ならびに, 消費電力で評価する.

  % 「コンパイラの最適化レベル」という単語が急に出てきて何のことか文章だけではわかりにくいので、説明を入れる
